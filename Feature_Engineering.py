...
# drop first sayesinde 2 sınıflı değişkenleri label encoding yapabiliyoruz
# get dummies metodu ile 2 sınıflı değişkenleri hem label encoding
# hemde one-hot encoding yöntemi için kullanılabilir
...
df = load()
df.head()
# notnull dolu mu ? sorusunu soruyor bool tipi cevap gelicek
# astype ile integer a çeviriliyor
...
# count başarı  durumu(hayatta kalma durumu) , nobs gözlem durumu
...
# burda mean değerlerinde farklılık var
# bu farklılığı aşağıda istatiski açıdan olup olmadığına bakıyoruz
...
# çıkan p-value değerine göre yorum yapılır
#############################################
